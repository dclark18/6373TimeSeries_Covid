...
as2Small = as2[1:130,]
as2Large = as2[1:120,]
as2SmallDF =  data.frame(test = ts(as2Small$totalTestResultsIncrease, frequency = 1), positive = ts(as2Small$positiveIncrease, frequency = 1))
as2LargeDF =  data.frame(test = ts(as2Large$totalTestResultsIncrease, frequency = 1), positive = ts(as2Large$positiveIncrease, frequency = 1))
```

From here, we will fit it into a neural net model.

```{r}
fit.mlp.small = mlp(ts(as2Small$posper, frequency = 1), allow.det.season = FALSE, reps = 20, comb = 'mean', xreg = as2SmallDF)
#fit.mlp.small = mlp(ts(us2Small$deathIncrease, frequency = 1), allow.det.season = FALSE, reps = 20, comb = 'mean', xreg = us2SmallDF)
fit.mlp.small
plot(fit.mlp.small)

fit.mlp.large = mlp(ts(as2Large$posper, frequency = 1), allow.det.season = FALSE, reps = 20, comb = 'mean', xreg = as2LargeDF)
#fit.mlp.large = mlp(ts(us2Large$deathIncrease, frequency = 1), allow.det.season = FALSE, reps = 20, comb = 'mean', xreg = us2LargeDF)
fit.mlp.large
plot(fit.mlp.large)
```
...
```{r}
ASDF = data.frame(test = ts(as2$totalTestResults), positive = ts(as2$positiveIncrease))
for.mlp10 = forecast(fit.mlp.small, h = 10, xreg = ASDF)
fore.mlp20 = forecast(fit.mlp.large, h = 20, xreg = ASDF)
plot(fore.mlp10)
plot(fore.mlp20)
```

Our ASE ranges widely within our 20 repititions. 

Now let's check the ASE of our short term and long term forecast. 

```{r}
ASEmlp10 = mean((as2$posper[131:140] - fore.mlp10$mean)^2)
ASEmlp10
ASEmlp20 = mean((as2$posper[121:140] - fore.mlp20$mean)^2)
ASEmlp20
```
...
```{r}
ensemble10 = (predsar10$fcst$y1[,1] + fore.mlp10$mean)/2
ensemble20 = (predsar20$fcst$y1[,1] + fore.mlp20$mean)/2

```

```{r}
plot(seq(1,140,1), as2$posper, type = "l",xlim = c(0,145), ylim = c(0,0.5), ylab = "Positive Percentage", main = "10 Day Positive Percentage Forecast Ensemble")
lines(seq(131,140,1), ensemble10, type = "l", col = "red")

plot(seq(1,140,1), as2$posper, type = "l",xlim = c(0,145), ylim = c(0,0.5), ylab = "Positive Percentage", main = "20 Day Positive Percentage Forecast Ensemble")
lines(seq(121,140,1), ensemble20, type = "l", col = "red")
```

Ensemble ASE

```{r}
ASE10 = mean((as2$posper[131:140] - ensemble10)^2)
ASE10

ASE20 = mean((as2$posper[121:140] - ensemble20)^2)
ASE20
```
